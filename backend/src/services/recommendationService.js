...
function tagsOf(d) {
  return parseTags(d.tipoTurismoPrincipal);
}
...
function avgIndex(values, order) {
  const idxs = values
    .map((v) => order.indexOf(parseSingle(v)))
    .filter((i) => i >= 0);
...
async function getRecommendations(userId, limit = 8) {
  const [favs, colItems, user] = await Promise.all([
    ...
    prisma.collectionItem.findMany({
      where: {
        collection: { OR: [{ userId }, { members: { some: { userId } } }] },
      },
      select: { destinoId: true },
    }),
  ...
  const likedIds = [
    ...new Set([
      ...favs.map((f) => f.destinoId),
      ...colItems.map((c) => c.destinoId),
    ]),
  ];

  const travel = (user?.preferences && user.preferences.travel) || {};
  const prefTipos = Array.isArray(travel.tipos) ? travel.tipos : [];
  const prefPresupuesto =
    typeof travel.presupuesto === "string" ? travel.presupuesto : null;
  ...
  liked.forEach((d) =>
    tagsOf(d).forEach((t) => tagWeights.set(t, (tagWeights.get(t) || 0) + 1)),
  );
  prefTipos.forEach((t) => tagWeights.set(t, (tagWeights.get(t) || 0) + 2));

  let preferredPresupuestoIdx = prefPresupuesto
    ? PRESUPUESTO_ORDER.indexOf(prefPresupuesto)
    : -1;
  if (preferredPresupuestoIdx < 0 && liked.length) {
    preferredPresupuestoIdx = avgIndex(
      liked.map((d) => d.presupuesto),
      PRESUPUESTO_ORDER,
    );
  }
  const preferredMasIdx = liked.length
    ? avgIndex(
        liked.map((d) => d.masificacion),
        MASIFICACION_ORDER,
      )
    : -1;
  ...
  const scored = candidates.map((d) => {
    ...
    const tagScoreRaw = overlap.reduce(
      (s, t) => s + (tagWeights.get(t) || 0),
      0,
    );
    ...
    else if (preferredMasIdx >= 0 && candMasIdx >= 0)
      masScore = 1 - Math.abs(candMasIdx - preferredMasIdx) / 4;
