...
export function ItineraryBuilder({ collection, canEdit = false, onSave }: ItineraryBuilderProps) {
  const initialDays = collection.itinerary.length
    ? normalizeDays(collection.itinerary, collection.startDate)
  ...
  useEffect(() => {
    setDays(
      collection.itinerary.length
        ? normalizeDays(collection.itinerary, collection.startDate)
        : generateItinerary(collection),
    );
  }, [collection.itinerary, collection.items, collection.startDate, collection.endDate]);
  ...
  const savedDays = collection.itinerary.length
    ? normalizeDays(collection.itinerary, collection.startDate)
    : generateItinerary(collection);
