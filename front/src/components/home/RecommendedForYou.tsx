    ...
    <section className="max-w-7xl mx-auto px-4 sm:px-6 md:px-10 py-12 sm:py-16">
      <ScrollReveal>
        <div className="flex items-center gap-2.5 mb-2">
          ...
          <h2 className="font-serif text-2xl sm:text-3xl md:text-4xl text-[var(--color-primary)] tracking-tight">Para ti</h2>
        </div>
        <p className="text-[var(--color-muted)] text-sm sm:text-base mb-6 sm:mb-8">Recomendaciones basadas en tus favoritos y preferencias de viaje.</p>
      </ScrollReveal>

      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-6">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="h-56 sm:h-80 rounded-2xl bg-[var(--color-surface)] border border-[var(--color-border)] animate-pulse" />
          ))}
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-6">
